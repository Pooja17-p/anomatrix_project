        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
              <span>Recent Mouse Telemetry & Evaluation Activity</span>
            ...
            <p className="text-xs text-slate-400">
              Historical window telemetry captured and analyzed by AI engine
            </p>
          </div>
        </div>
        ...
        <table className="w-full text-left text-xs text-slate-300">
          <thead className="bg-slate-950 uppercase text-slate-400 text-[11px] font-semibold tracking-wider border-b border-slate-800">
            <tr>
              ...
              <th className="py-3.5 px-4">Points Tracked</th>
              <th className="py-3.5 px-4">Avg Speed</th>
              <th className="py-3.5 px-4">Peak Speed</th>
              <th className="py-3.5 px-4">Clicks / Scrolls</th>
            ...
            {filteredHistory.length === 0 ? (
              <tr>
                <td colSpan="8" className="text-center py-8 text-slate-500 font-sans">
                  No telemetry logs matching criteria. Move cursor to generate data.
                </td>
              </tr>
            ) : (
              filteredHistory.map((row) => {
                ...
                return (
                  <tr key={row.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-3 px-4 text-cyan-300 font-semibold">{row.pointsCaptured} pts</td>
                    <td className="py-3 px-4">{row.avgSpeed} px/s</td>
                    <td className="py-3 px-4 text-purple-300">{row.peakSpeed} px/s</td>
                    <td className="py-3 px-4 text-slate-300">
                      {row.clicks} clicks | {row.scrolls} scrolls
                    </td>
